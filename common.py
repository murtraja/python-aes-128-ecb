...
class Common_Operations:
    ...
    def make_matrix_for_mixcolumns(self, row):
        '''
        this function takes the first row of the matrix and generates the rest
        '''
        matrix = [self.rotate_bytes_left(row, i) for i in range(len(row), 0, -1)]
        return matrix
    
    def multiply_row_column_for_mixcolumns(self, row, column):
        new_value = 0
        for i in range(len(column)):
            new_value = new_value ^ self.mixcolumns[row[i]][column[i]]
        return new_value
    
    def multiply_matrix_for_mixcolumns(self, matrix, column):
        new_column = [0 for _ in column]
        for i in range(len(column)):
            new_value = self.multiply_row_column_for_mixcolumns(matrix[i], column)
...
if __name__ == "__main__":
    com = Common_Operations()
    matrix = com.make_matrix_for_mixcolumns([14,11,13,9])
    print [com.multiply_row_column_for_mixcolumns(matrix[i], [142,77,161,188]) for i in range(4)]
